...
添加 `@classmethod` 装饰器能够使方法成为类方法，不需要实例化对象，也可以直接通过类名调用；与静态方法不同的是，类方法会自动接收类本身作为第一个参数，通常命名为 `cls`

```python
class Something:
    class_value = 1  # 类属性 (具体介绍见本页“属性”部分)

    def __init__(self, value):
        self.value = value

    @classmethod
    def something_class(cls, val):
        return cls.class_value + val

# 注意这里并没有实例化Something类
ans = Something.something_class(2)
print(ans)
```

`cls` 能够传入当前调用的方法对应的类，因此类方法适合用于：
1. 访问或修改类属性 (示例中展示)
2. 根据类本身创建对象
