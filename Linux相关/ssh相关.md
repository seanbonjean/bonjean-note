...
作用：把**本地某个端口**的流量，通过SSH转发到**远端的某个地址和端口**，从而在本地端口就能访问到远程端口

```bash
ssh -L 本机端口:远程ip:远程端口 sean@192.168.1.11
```

这里的远程ip可以是 `localhost` ，也可以是远程服务器能够访问到的另一台机器

情景1：本地访问远程服务器网页 `ssh -L 12345:localhost:443 sean@100.101.102.103`

情景2：通过暴露在公网的跳板机，访问内网服务器的8888端口 `ssh -L 12345:192.168.1.100:8888 sean@100.101.102.103`

反过来，用 `-R` 能把远程端口转发到本地： `ssh -R 远程端口:目标ip:目标端口 sean@192.168.1.11`
